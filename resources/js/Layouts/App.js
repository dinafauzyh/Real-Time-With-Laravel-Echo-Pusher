...
export default function App({title, children}){
    ...
    return (
        <div className="flex min-h-screen">
            <Head title={title}/>
                <div className="w-1/3">
                    <div className="w-1/3 fixed h-full flex flex-col px-6 py-4 text-right border-r space-y-3">
                        ...
                        <div className="flex-1 overflow-y-auto">
                            {users.map(user => (
                                <Link 
                                    ...
                                    className={`block ${route().current('chats.show', user.username) ? 'text-black font-semibold' : 'text-gray-600'}`}
                                >
                                        {user.name}
                                </Link>
                            ))}
